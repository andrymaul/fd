...
export async function runEnrichmentPhase1() {
  const targetFile = path.join(process.cwd(), 'src', 'data', 'ddinter2Phase1ChronicAdditions.ts');
  const backupFile = path.join(process.cwd(), 'src', 'data', 'ddinter2Phase1ChronicAdditions.ts.bak');
  let content = fs.readFileSync(targetFile, 'utf8');

  console.log('========================================================================');
  console.log('🚀 TAHAP A: ENRICHMENT OBAT KRONIS & KARDIOVASKULAR (PHASE 1 CHRONIC)');
  console.log('========================================================================\n');
  ...
  console.log(`Ditemukan ${targets.length} interaksi kronis dengan server ID DDInter yang perlu diperkaya.\n`);

  if (targets.length === 0) {
    console.log('Semua interaksi Phase 1 sudah memiliki teks asli DDInter Management!');
    return;
  }
  ...
  console.log(`\n🎉 TAHAP A SELESAI! Sebanyak ${enrichedCount} interaksi kronis & kardiovaskular berhasil diperkaya dengan teks asli Management DDInter 2.0!\n`);
}

runEnrichmentPhase1().catch(err => {
  console.error('Error during Phase 1 enrichment:', err);
  process.exit(1);
});
