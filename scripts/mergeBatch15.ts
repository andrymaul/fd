...
const BATCH15_FILE = path.join(process.cwd(), 'data', 'batch15_extracted.json');
const TARGET_TS_FILE = path.join(process.cwd(), 'src', 'data', 'ddinter2ScrapedInteractions.ts');

if (!fs.existsSync(BATCH15_FILE)) {
  console.error('Batch 15 file not found:', BATCH15_FILE);
  process.exit(1);
}
...
// 2. Read Batch 15 records
const batch15Raw = JSON.parse(fs.readFileSync(BATCH15_FILE, 'utf8'));
console.log(`Batch 15 raw records to merge: ${batch15Raw.length}`);
...
const FALLBACK_ALTERNATIVES: Record<string, string[]> = {
  'Abametapir (topical)': ['Permethrin (topical)', 'Ivermectin (topical)', 'Spinosad', 'Malathion (topical)', 'Benzyl alcohol'],
  'Cocaine (topical)': ['Lidocaine (topical)', 'Tetracaine (topical)', 'Bupivacaine', 'Prilocaine'],
  'Conivaptan': ['Tolvaptan', 'Furosemide', 'Spironolactone'],
  'Deflazacort': ['Prednisone', 'Methylprednisolone', 'Hydrocortisone', 'Dexamethasone']
};
...
const newInteractions: DrugInteraction[] = batch15Raw.map((item: any) => {
  const slugA = item.drugAName.toLowerCase().replace(/[^a-z0-9]/g, '-');
  const slugB = item.drugBName.toLowerCase().replace(/[^a-z0-9]/g, '-');

  let altsA = (item.alternativeOptionsA || []).map(cleanEntities).filter(Boolean);
  if (altsA.length === 0 && FALLBACK_ALTERNATIVES[item.drugAName]) {
    altsA = [...FALLBACK_ALTERNATIVES[item.drugAName]];
  }

  let altsB = (item.alternativeOptionsB || []).map(cleanEntities).filter(Boolean);
  if (altsB.length === 0 && FALLBACK_ALTERNATIVES[item.drugBName]) {
    altsB = [...FALLBACK_ALTERNATIVES[item.drugBName]];
  }
 ...
 * Total Ingested Pairs: ${mergedList.length} (Batch 1-15 Complete)
 */
