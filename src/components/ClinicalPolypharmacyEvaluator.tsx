...
export const ClinicalPolypharmacyEvaluator: React.FC<ClinicalPolypharmacyEvaluatorProps> = ({
  ...
  // Comorbidity Dropdown & Filter state
  const [isComorbidityDropdownOpen, setIsComorbidityDropdownOpen] = useState<boolean>(false);
  const [comorbiditySearchTerm, setComorbiditySearchTerm] = useState<string>('');
  const [customComorbidityInput, setCustomComorbidityInput] = useState<string>('');

  const filteredComorbidityOptions = useMemo(() => {
    if (!comorbiditySearchTerm.trim()) return COMORBIDITY_OPTIONS;
    const q = comorbiditySearchTerm.toLowerCase();
    return COMORBIDITY_OPTIONS.filter(item => item.toLowerCase().includes(q));
  }, [comorbiditySearchTerm]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-[#130924] p-6 rounded-3xl border border-violet-200/80 dark:border-violet-500/25 shadow-xs space-y-5">
            ...
            <div className="space-y-4 text-xs">
              ...
              {/* Komorbiditas & Kondisi Medis Pasien (Dropdown Multi-Select) */}
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-2 relative">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-slate-800 dark:text-slate-200 block text-[11px] flex items-center gap-1.5">
                    <Heart className="w-4 h-4 text-rose-500" />
                    <span>Komorbiditas & Riwayat Medis</span>
                  </span>
                  <div className="flex items-center gap-1.5">
                    {patient.comorbidities && patient.comorbidities.length > 0 && (
                      <button
                        type="button"
                        onClick={() => setPatient({ ...patient, comorbidities: [] })}
                        className="text-[9.5px] text-rose-600 hover:text-rose-800 dark:text-rose-400 font-semibold hover:underline cursor-pointer"
                      >
                        Reset
                      </button>
                    )}
                    <span className="text-[10px] font-black text-teal-700 dark:text-teal-300 px-1.5 py-0.5 rounded-full bg-teal-100 dark:bg-teal-900/50">
                      {patient.comorbidities?.length || 0} Terpilih
                    </span>
                  </div>
                </div>
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsComorbidityDropdownOpen(!isComorbidityDropdownOpen)}
                    className="w-full px-3 py-2 text-left bg-white dark:bg-slate-900 border border-teal-200 dark:border-teal-800/80 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 flex items-center justify-between shadow-2xs hover:border-teal-400 focus:outline-none focus:ring-2 focus:ring-teal-400/40 cursor-pointer"
                  >
                    <span className="truncate text-slate-600 dark:text-slate-300">
                      {patient.comorbidities && patient.comorbidities.length > 0
                        ? `${patient.comorbidities.length} Komorbiditas Dipilih (Klik untuk ubah/tambah)`
                        : '+ Pilih Komorbiditas & Penyakit...'}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${isComorbidityDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>

                  {/* Backdrop for click outside */}
                  {isComorbidityDropdownOpen && (
                    <div 
                      className="fixed inset-0 z-20" 
                      onClick={() => setIsComorbidityDropdownOpen(false)} 
                    />
                  )}

                  {/* Dropdown Menu */}
                  {isComorbidityDropdownOpen && (
                    <div className="absolute z-30 left-0 right-0 mt-1 p-2.5 bg-white dark:bg-slate-900 border border-teal-200 dark:border-teal-800 rounded-xl shadow-xl space-y-2 animate-in fade-in zoom-in-95 max-h-72 flex flex-col">
                      {/* Search in dropdown */}
                      <div className="relative">
                        <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
                        <input
                          type="text"
                          placeholder="Cari komorbiditas / riwayat penyakit..."
                          value={comorbiditySearchTerm}
                          onChange={(e) => setComorbiditySearchTerm(e.target.value)}
                          className="w-full pl-8 pr-2 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-teal-400"
                        />
                      </div>

                      {/* Options List */}
                      <div className="overflow-y-auto flex-1 space-y-1 max-h-44 pr-1">
                        {filteredComorbidityOptions.map((item, idx) => {
                          const isSelected = patient.comorbidities?.includes(item);
                          return (
                            <button
                              key={idx}
                              type="button"
                        ...
                        {filteredComorbidityOptions.length === 0 && (
                          <div className="p-3 text-center text-xs text-slate-400 italic">
                            Tidak ditemukan dalam daftar standar.
                          </div>
                        )}
                      ...
                      <div className="pt-1.5 border-t border-slate-100 dark:border-slate-800 flex items-center gap-1.5">
                        <input
                          type="text"
                          placeholder="Komorbiditas lain (ketik manual)..."
                          value={customComorbidityInput}
                          onChange={(e) => setCustomComorbidityInput(e.target.value)}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' && customComorbidityInput.trim()) {
                              e.preventDefault();
                              if (!patient.comorbidities?.includes(customComorbidityInput.trim())) {
                                setPatient({
                                  ...patient,
                                  comorbidities: [...(patient.comorbidities || []), customComorbidityInput.trim()]
                                });
                              }
                              setCustomComorbidityInput('');
                            }
                          }}
                          className="flex-1 px-2 py-1 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-teal-400"
                        />
                        <button
                          type="button"
                          onClick={() => {
                            if (customComorbidityInput.trim()) {
                              if (!patient.comorbidities?.includes(customComorbidityInput.trim())) {
                                setPatient({
                                  ...patient,
                                  comorbidities: [...(patient.comorbidities || []), customComorbidityInput.trim()]
                          ...
                          className="px-2.5 py-1 bg-teal-600 hover:bg-teal-700 text-white text-xs font-bold rounded-lg cursor-pointer shrink-0"
                        >
                          + Tambah
                        </button>
                      </div>
                ...
                {/* Selected Comorbidities Badges */}
                {patient.comorbidities && patient.comorbidities.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {patient.comorbidities.map((item, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10.5px] font-bold bg-teal-50 text-teal-800 dark:bg-teal-950/60 dark:text-teal-200 border border-teal-200 dark:border-teal-800"
                      >
                        <span className="max-w-[200px] truncate">{item}</span>
                        <button
                          type="button"
                          onClick={() => handleToggleComorbidity(item)}
                          className="hover:text-teal-950 dark:hover:text-white p-0.5 rounded cursor-pointer font-black text-xs"
                          title="Hapus komorbiditas ini"
                        >
                          ×
                        </button>
                      </span>
