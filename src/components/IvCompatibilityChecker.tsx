        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-[#071726] border border-sky-200/80 dark:border-sky-500/25 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-sky-100 dark:border-sky-950/80">
              ...
              <div>
                {overallYSiteStatus === 'insufficient' && (
                  <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold font-outfit bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                    <Info className="w-4 h-4 text-sky-500" />
                    PILIH MINIMAL 2 OBAT UNTUK SKRINING
                  </span>
                )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {/* Searchable Add Drug Dropdown */}
              <div className="relative" ref={ySiteDropdownRef}>
                <button
                  type="button"
                  id="btn-add-ysite-drug"
                  onClick={() => setIsYSiteDropdownOpen(prev => !prev)}
                  className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold font-outfit focus:outline-none cursor-pointer transition-all shadow-2xs border ${
                    isYSiteDropdownOpen
                      ? 'bg-sky-500 text-white border-sky-600 shadow-md shadow-sky-500/20 ring-2 ring-sky-400/40'
                      : 'bg-sky-500/10 hover:bg-sky-500/15 text-sky-900 dark:text-sky-200 border-sky-400/40 hover:border-sky-400/70'
                  }`}
                >
                  <Search className={`w-3.5 h-3.5 ${isYSiteDropdownOpen ? 'text-white' : 'text-sky-500'}`} />
                  <span>+ Tambah Obat Injeksi...</span>
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ml-0.5 ${isYSiteDropdownOpen ? 'rotate-180 text-white' : 'text-sky-600 dark:text-sky-400'}`} />
                </button>

                {/* Dropdown Popover */}
                {isYSiteDropdownOpen && (
                  <div
                    className="absolute left-0 top-full mt-2 z-50 w-[330px] sm:w-[420px] max-w-[calc(100vw-2rem)] bg-white dark:bg-[#091e32] border border-sky-200 dark:border-sky-700/80 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150"
                  >
                    {/* Header with Search Input */}
                    <div className="p-3 border-b border-sky-100 dark:border-sky-950 bg-slate-50/70 dark:bg-slate-900/60 space-y-2">
                      <div className="relative flex items-center">
                        <Search className="w-4 h-4 text-sky-500 absolute left-3 pointer-events-none" />
                        <input
                          ...
                          placeholder="Cari obat (cth: Norepi, Furo, Pantop, Kategori)..."
                          className="w-full bg-white dark:bg-slate-950 text-slate-900 dark:text-white border border-sky-200 dark:border-sky-800 rounded-xl pl-9 pr-8 py-2 text-xs font-semibold font-outfit focus:outline-none focus:ring-2 focus:ring-sky-500 placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-2xs"
                        />
                        {ySiteSearchQuery && (
                          <button
                            type="button"
                      ...
                      <div className="flex items-center gap-1 overflow-x-auto pb-1 no-scrollbar text-[10px] font-bold font-outfit">
                        {ySiteCategories.slice(0, 6).map((cat) => (
                          <button
                            key={cat}
                            type="button"
                            onClick={() => {
                            ...
                            className={`px-2 py-0.5 rounded-lg whitespace-nowrap transition cursor-pointer ${
                              ySiteCategoryFilter === cat
                                ...
                                : 'bg-slate-200/70 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-sky-100 dark:hover:bg-sky-950/60'
                            }`}
                          >
                            {cat}
                          </button>
                      ...
                      <div className="flex items-center justify-between text-[10.5px] text-slate-400 dark:text-slate-400 font-medium px-0.5">
                        <span>Ditemukan <strong className="text-sky-600 dark:text-sky-400">{filteredYSiteDrugs.length}</strong> obat</span>
                        <span className="text-[10px] text-slate-400">Tekan <kbd className="px-1 py-0.2 rounded bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[9px] font-mono">Enter</kbd> untuk pilih</span>
                      </div>
                    </div>
                    ...
                    <div className="max-h-64 overflow-y-auto p-1.5 space-y-1 divide-y divide-slate-100 dark:divide-slate-800/40">
                      {filteredYSiteDrugs.length > 0 ? (
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between gap-2.5 p-2 rounded-xl text-left transition cursor-pointer ${
                                isHighlighted
                                  ? 'bg-sky-500/15 dark:bg-sky-500/20 text-sky-950 dark:text-sky-100 border border-sky-400/40'
                                  : 'hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-800 dark:text-slate-200 border border-transparent'
                              }`}
                            >
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="text-xs font-bold font-outfit text-slate-900 dark:text-white truncate">
                                    {drug.name}
                                  </span>
                                  <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-sky-500/10 text-sky-600 dark:text-sky-400 font-mono font-bold">
                                    pH {drug.phRange}
                                  </span>
                                </div>
                                <div className="text-[10.5px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5 mt-0.5 truncate">
                                  <span>{drug.category}</span>
                                  {drug.brandNames && drug.brandNames.length > 0 && (
                                    <>
                                      <span>•</span>
                                      <span className="italic truncate">{drug.brandNames.slice(0, 2).join(', ')}</span>
                                    </>
                                  )}
                                </div>
                              </div>
                        ...
                        <div className="py-8 text-center space-y-1.5">
                          <FlaskConical className="w-7 h-7 mx-auto text-slate-300 dark:text-slate-600" />
                          <p className="text-xs font-bold text-slate-600 dark:text-slate-400 font-outfit">
                            Obat injeksi tidak ditemukan
                          </p>
                          <p className="text-[11px] text-slate-400 dark:text-slate-500 max-w-[240px] mx-auto">
                            Coba kata kunci lain atau pilih kategori yang sesuai
                          </p>
                          {ySiteSearchQuery && (
                            <button
                              type="button"
                              ...
                              className="mt-2 text-xs text-sky-600 dark:text-sky-400 hover:underline font-bold font-outfit cursor-pointer"
                            >
                              Reset Pencarian
                            </button>
                          )}
              ...
              {selectedYSiteDrugIds.length > 0 && (
                <button
                  ...
                  className="text-[11px] font-bold font-outfit text-slate-400 hover:text-rose-500 dark:hover:text-rose-400 transition cursor-pointer px-2 py-1 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/40 ml-auto sm:ml-0"
                  title="Hapus semua obat terpilih"
                >
                  Reset Pilihan ({selectedYSiteDrugIds.length})
                </button>
              )}

              {selectedYSiteDrugIds.length === 0 && (
                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium italic">
                  Belum ada obat yang dipilih. Klik tombol di atas untuk mencari dan menambahkan obat injeksi.
                </span>
              )}
            </div>
