...
export const ClinicalToxicologyManager: React.FC<ClinicalToxicologyManagerProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12 font-outfit">
          ...
          <div className="bg-black/60 backdrop-blur-md p-4 rounded-2xl border border-rose-500/40 space-y-2 lg:w-72 shrink-0">
            <div className="flex items-center justify-between text-xs font-bold text-rose-300 border-b border-rose-800/80 pb-2">
              ...
              <span className="bg-rose-950 text-rose-300 px-2 py-0.5 rounded-full text-[10px] font-black border border-rose-600/40">
                {TOXICOLOGY_ANTIDOTES_DATABASE.length} Antidotum
              </span>
            </div>
            <div className="text-xs text-rose-100/80 space-y-1">
              <div className="flex justify-between">
                <span>Total Racun Kritis:</span>
                <span className="font-mono font-bold text-white">{TOXICOLOGY_ANTIDOTES_DATABASE.length} Agen</span>
              </div>
              <div className="flex justify-between">
      ...
      {/* 2. NAVIGATION TABS: DIRECTORY VS TOXICOLOGY CALCULATORS */}
      <div className="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-[#061427] rounded-2xl border border-slate-200 dark:border-slate-800 text-xs font-bold font-outfit w-full sm:w-fit">
        <button
          onClick={() => setActiveTab('directory')}
          className={`flex-1 sm:flex-none px-4 py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 ${
            activeTab === 'directory'
        ...
        >
          ...
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${activeTab === 'directory' ? 'bg-white/20 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
        ...
        <button
          ...
          className={`flex-1 sm:flex-none px-4 py-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 ${
            activeTab === 'calculators'
              ? 'bg-gradient-to-r from-amber-600 to-rose-600 text-white shadow-md shadow-amber-950/30'
              : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
          }`}
        >
          <Calculator className="w-4 h-4" />
          <span>Kalkulator Toksikologi &amp; Nomogram</span>
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono ${activeTab === 'calculators' ? 'bg-white/20 text-white' : 'bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300'}`}>
            3 Tool EBM
          </span>
        </button>
      </div>
